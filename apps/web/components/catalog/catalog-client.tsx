...
export function CatalogClient() {
  ...
  const clearAll = () => {
    ...
    setPriceMax(priceRange.max || 3500);
    setSort('featured');
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#FAFAF7' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 pb-20">
        ...
        {/* Active Filters */}
        {(search || selectedCats.size > 0 || selectedBrands.size > 0 || ecoOnly || brandingOnly || priceMin > (priceRange.min || 0) || priceMax < (priceRange.max || 3500)) && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {(priceMin > (priceRange.min || 0) || priceMax < (priceRange.max || 3500)) && <button onClick={() => { setPriceMin(priceRange.min || 0); setPriceMax(priceRange.max || 3500); }} className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium rounded-full" style={{ background: '#E8F5EF', color: '#0F4934' }}>Price: {formatPrice(priceMin)}–{formatPrice(priceMax)} ✕</button>}
              ...
              <div className="mb-4 pb-3 border-b">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input type="range" min={priceRange.min || 0} max={priceRange.max || 3500} value={priceMin} onChange={(e) => setPriceMin(Math.min(parseInt(e.target.value), priceMax - 100))} className="w-full" style={{ accentColor: '#1A6B4F' }} />
                    <input type="range" min={priceRange.min || 0} max={priceRange.max || 3500} value={priceMax} onChange={(e) => setPriceMax(Math.max(parseInt(e.target.value), priceMin + 100))} className="w-full" style={{ accentColor: '#1A6B4F' }} />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {filtered.map(p => (
                  <Link key={p.id} href={`/products/${p.slug}`} className="bg-white rounded-md shadow hover:shadow-lg hover:translate-y-[-4px] transition overflow-hidden cursor-pointer block">
                    ...
                    <button className="w-[calc(100%-28px)] mx-3.5 h-8 bg-emerald-700 text-white text-xs font-semibold rounded-full mb-3 hover:bg-emerald-800 transition">Add to Pack</button>
                  </Link>
                ))}
              </div>
            )}
