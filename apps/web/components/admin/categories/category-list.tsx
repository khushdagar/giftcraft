...
export function CategoryList({ categories }: { categories: CategoryRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return categories.filter(
      (c) =>
        ...
        (c.parent?.name.toLowerCase().includes(q) ?? false)
    );
  }, [categories, query]);
  ...
  const deleteSelected = async () => {
    ...
    if (!confirm(`Delete ${ids.length} categor${ids.length === 1 ? 'y' : 'ies'}? Products are kept and simply un-categorised.`)) return;
    setDeleting(true);
    try {
      const results = await Promise.allSettled(
        ids.map((id) => fetch(`/api/admin/categories/${id}`, { method: 'DELETE' }))
      );
      const failed = results.filter((r) => r.status === 'rejected' || (r.status === 'fulfilled' && !r.value.ok)).length;
      if (failed) toast.error(`${failed} could not be deleted`);
      else toast.success(`Deleted ${ids.length} categor${ids.length === 1 ? 'y' : 'ies'}`);
      setSelected(new Set());
      router.refresh();
    } catch {
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-2">{categories.length} categor{categories.length === 1 ? 'y' : 'ies'}</p>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-bdr bg-white shadow-sm">
        ...
        <div className="flex items-center gap-3 border-b border-bdr bg-gray-50/60 px-4 py-2 text-xs font-medium text-ink-2">
          ...
          <span className="w-56">Parent</span>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-bdr bg-gray-50">
                    {cat.imageUrl ? (
                      <Image src={cat.imageUrl} alt="" width={36} height={36} className="h-full w-full object-cover" />
                    ) : (
                  ...
                  <Link href={`/admin/categories/${cat.id}/edit`} className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-sky-700 group-hover:underline">{cat.name}</div>
                    <div className="truncate text-xs text-ink-3">
                      {cat.parent ? `Sub-category of · ${cat.parent.name}` : 'Top-level'}
                    </div>
                  </Link>

                  {/* Product count */}
                  <div className="w-24 text-right text-sm tabular-nums text-ink">{cat._count.products}</div>

                  {/* Parent column */}
                  <div className="w-56 truncate text-sm text-ink-2">
                    {cat.parent ? cat.parent.name : <span className="text-ink-3">—</span>}
                  </div>
                </li>
              );
            })}
      ...
      <p className="mt-4 text-center text-xs text-ink-3">
        Categories group your products. Click one to edit its details, image and members.
      </p>
    </div>
  );
}
