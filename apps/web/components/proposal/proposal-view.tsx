...
export interface ProposalViewPack {
  id: string;
  label: string;
  tagline: string | null;
  /** Quote share token. Null in preview — nothing has been created yet. */
  token: string | null;
  payload: any;
}

/**
 * The proposal comparison page, as the recipient sees it.
 *
 * Shared by /proposal/[token] and the admin's "Preview proposal" dialog, so
 * what an admin checks before sending is the same markup and the same priced
 * payload the client will open — not a second rendering that can drift.
 *
 * In `preview` mode the per-pack CTAs and PDF links are inert: no quote tokens
 * exist until the proposal is actually sent.
 */
export function ProposalView({
  ...
  /** Short proposal id shown in the header. Omitted in preview. */
  reference?: string;
  /** Full-proposal PDF URL. Omitted in preview — the deck needs a real token. */
  deckHref?: string;
  recipientName?: string | null;
  companyName?: string | null;
  message?: string | null;
  expiresAt: Date;
  packs: ProposalViewPack[];
  preview?: boolean;
}) {
      ...
      <div className="container-gc-w max-w-7xl">
        ...
        <div className="mb-8 max-w-2xl">
          <p className="text-xs font-normal uppercase tracking-wider text-ink-3">
            {reference ? `Proposal #${reference}` : 'Proposal preview'}
          </p>
          <h1 className="mt-2 text-3xl sm:text-4xl font-normal text-ink">
            {recipientName ? `${recipientName}, your` : 'Your'} gift pack
            {isMulti ? ' options' : ''}
          </h1>
        ...
        {message && (
          <div className="mb-8 rounded-md border-2 border-bdr bg-white p-5 max-w-3xl">
            ...
            <p className="text-sm text-ink-2 whitespace-pre-line leading-relaxed">{message}</p>
          </div>
        )}
              ...
              >
                ...
                <div className="border-t border-bdr bg-gray-50 p-5 space-y-2">
                  ...
                  <div className="pt-3 space-y-2">
                    {preview || !pack.token ? (
                      // No quote exists yet — show the button the client will
                      // get, but inert.
                      <Button
                        variant="em"
                        size="lg"
        ...
        <div className="mt-8 rounded-md border border-blue-200 bg-blue-50 p-4 max-w-3xl">
          <p className="text-xs text-blue-900 leading-relaxed">
            Shipping is calculated at checkout once you enter the delivery address. Every price
            shown includes GST and the 2% payment-processing fee. Want a pack between these
            options? Reply to the proposal email and we&apos;ll adjust it.
          </p>
        </div>
      </div>
    </div>
