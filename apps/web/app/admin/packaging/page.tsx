  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-normal">Packaging Options</h1>
            ...
            <tbody className="divide-y">
              {packaging.map((pkg) => (
                <tr key={pkg.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    {pkg.price === 0 ? <span className="text-green-600 font-normal">Free</span> : <span className="font-normal">₹{pkg.price}</span>}
                  </td>
                  <td className="px-6 py-4 text-sm">{pkg.sortOrder}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-normal ${pkg.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
