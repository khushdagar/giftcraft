...
/**
 * Build the deck's itemised GST table from the quote payload — the SAME rows
 ...
 * reverse-calculated. Returns null when the quote carries no pricing snapshot.
 */
function buildDeckInvoice(payload: any): DeckInvoice | null {
  ...
  // 2. Packaging & add-ons — priced per gift pack (HSN 4819, 18%).
  ...
  const addonsTotal = round2(addonsPerUnit * packQuantity);
  if (addonsTotal > 0) {
...
export async function GET(
  ...
  try {
    ...
    if (payloadProducts.length === 0) {
      return new Response('This quote has no products to present', {
        status: 422,
      });
    }
    ...
    // master rather than the quote snapshot, so an admin improving a product's
    // copy immediately improves every deck generated afterwards. Pricing still
    // comes from the snapshot so the deck can never contradict the quote.
    ...
    // while prices stay from the quote snapshot. The packaging snapshot id is
    // synthetic — `<productId>-<size>` (see lib/packaging-designs) — so the
    // size suffix is stripped before hitting the Product table.
    const payloadAddons: any[] = Array.isArray(payload?.addons)
      ? payload.addons
      : [];
    ...
    const [clientLogo, packagingImage, imageData, addonImages] =
      await Promise.all([
        toDataUri(payload?.logoUrl),
        toDataUri(packagingProduct?.images[0]?.url),
        Promise.all(
          payloadProducts.map((p) => toDataUri(byId.get(p.id)?.images[0]?.url))
        ),
        Promise.all(
          payloadAddons.map((a) =>
            toDataUri(extraById.get(a.id)?.images[0]?.url)
          )
        ),
      ]);
    ...
    const companyName =
      payload?.address?.company?.trim() ||
      quote.company?.name ||
      quote.createdBy?.company?.name ||
      null;
